...
async def get_rate(
    ...
    # Load CostProfiles for fallback use
    profiles = None
    try:
    ...
    if component_clean == "telephony":
        ...
        if profiles:
            try:
                val = profiles.get_telephony_cost_per_minute(provider_clean)
                if val > 0:
                    return Decimal(str(val)), "minute", "provider_costs_yaml", False
            except Exception:
                pass
    ...
    if component_clean == "stt":
        ...
        if profiles:
            try:
                val = profiles.get_stt_cost_per_minute(provider_clean) / 60.0
                if val > 0:
                    return Decimal(str(val)), "second", "provider_costs_yaml", False
            except Exception:
                pass
    ...
    if component_clean == "llm":
        if profiles:
            try:
                val = profiles.get_llm_cost_per_1k_tokens(provider_clean, model_clean, is_output=False) / 1000.0
                if val > 0:
                    return Decimal(str(val)), "token", "provider_costs_yaml", False
            except Exception:
                pass
    ...
    if component_clean == "tts":
        ...
        if profiles:
            try:
                cost_char = profiles.costs.get("tts", {}).get(provider_clean, {}).get("cost_per_character", 0.0)
                if cost_char > 0:
                    return Decimal(str(cost_char)), "character", "provider_costs_yaml", False
            except Exception:
                pass
...
async def get_llm_rates(
    ...
    # Check central YAML config
    try:
        from dana.config.cost_profiles import CostProfiles
        profiles = CostProfiles()
        prompt_rate = profiles.get_llm_cost_per_1k_tokens(provider_clean, model_clean, is_output=False) / 1000.0
        completion_rate = profiles.get_llm_cost_per_1k_tokens(provider_clean, model_clean, is_output=True) / 1000.0
        if prompt_rate > 0 or completion_rate > 0:
            return Decimal(str(prompt_rate)), Decimal(str(completion_rate)), "provider_costs_yaml", False
    except Exception:
        pass
